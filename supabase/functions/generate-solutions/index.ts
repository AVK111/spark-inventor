...
// Hugging Face API integration with RAG query format
async function generateWithHuggingFace(problemDescription: string, huggingFaceApiKey: string) {
  const ragQuery = {
...
Respond in this exact JSON format:
{
  ...
  try {
    console.log('Executing RAG query with Hugging Face API...');
    ...
    // Extract JSON from response (Hugging Face sometimes includes extra text)
    ...
    if (!Array.isArray(solutions) || solutions.length === 0) {
      throw new Error('No valid solutions generated by Hugging Face');
    }

    console.log(`Hugging Face generated ${solutions.length} RAG-powered solutions`);
...
serve(async (req) => {
  ...
  try {
    ...
    // Prefer Hugging Face for real research, fallback to OpenAI, then mock data
    if (huggingFaceApiKey) {
      console.log('Using Hugging Face API for real research data');
      return await generateWithHuggingFace(problemDescription, huggingFaceApiKey);
